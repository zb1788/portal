...
<link rel="stylesheet" href="<%=path %>/space/module/dxekt/css/public.css?r=<%=dayStamp %>">
<link rel="stylesheet" href="<%=path %>/space/module/dxekt/css/edit.css?r=<%=dayStamp %>">
...
var indexlogo =  (portal_config.indexlogo!=""&&portal_config.indexlogo!=undefined)?portal_config.indexlogo:"dxekt_logo.jpg";
...
<script> 
$(function() {
    if ($.trim($('#menu li a.sel').text()) == "首页") {
        hideUserInfo();
    } else {
        showUserInfo();
    }
    $('#menu').find('a').click(function() {
        var isIndex = !!$(this).data('index');
...
<!-- 模拟alertDialog  -->
        ...
        <!-- 顶部 -->
        <div class="top">
            <div class="top-body w1200">
                <div class="top-left">
                    <a href="#" class="top-home">
                        <i class="i-ico i-ico-home"></i>
                        <span>翼课堂首页</span>
                    </a>
                    <span class="top-txt">您好，欢迎来到翼课堂！</span>
                ...
                <div class="top-right">
                    <ul>
                        ...
                        <li class="aid">
                            <a href="#" onclick="$('#login_con').show()">切换用户</a>
                        </li>
                        <li class="aid">
                            <a href="#" onClick="util.openIcode('CMS.PAGE.USEGUIDE');return false;">帮助</a>
                        </li>
                        <li>
    ...
    <DIV id=float_ad style="DISPLAY: none; RIGHT: 0px; WIDTH: 24px; CURSOR: pointer; TOP: 410px; POSITION: absolute;z-index:89">
...
<div class="head_wrap">
  <div class="<%=screenWidthId%>">
    <div class="yj_logo<%= ("4".equals(usertype)?" stu":"") %><%= ("0".equals(usertype)?" parent":"") %>"><img id="indexlogo" src="space/images/logo/dxekt_logo.jpg" /> <!-- <span><%=spaceName %></span> --></div>
    <div class="ssou_wrap" >
      <div class="ssou" >
        <div class="ssou_k">
          <input id="researchKey" class="txt_ssou"  placeholder="搜索您感兴趣的内容" type="text" onBlur="if (value ==''){value='搜索您感兴趣的内容'}"  onfocus="if (value =='搜索您感兴趣的内容'){value =''};" maxlength="10"/>
          <input type="submit" class="souBtn_white" value="搜索" onClick="researchKeyResource();" />
        </div>
      </div>
    </div>
    <!-- 搜索end-->
    <div class="userInfo fr" >
      ...
      <dl>
        <dt><!-- 头像及用户中心链接 -->
			<a onClick="util.openInFrame('TMS.CENTER');" >
        ...
        <dd class="posRe">
		<a class="tit1" onClick="util.openInFrame('TMS.CENTER');" title="<%=truename %>" ><%=showname %><%= ("4".equals(usertype)?"同学":"") %><%= (("2".equals(usertype)||"3".equals(usertype))?"老师":"") %></a>
		
          <!--弹出层-->
          <div class="login_con" id="login_con" >
            <div class="login_content">
                    <!-- 登录开始 -->
                 	  <form id="loginForm" name="loginForm" action="http://<%=Common.getSysUrl(request.getServerName(),"SSO") %>/sso/ssoAuth" method="get" target="">
              ...
              <div id="blo_1" style="display:;">
                ...
                <ul>
                  <li class="usr">
                    <div class="posRe">
                          <!-- 多账号选择层 -->
                          		<div id="roleDiv" phonenum="" class="ac_results" style="left:50px;top:40px;width:130px;display:none;position:absolute" >
                  ...
                  <li class="pwd">
                    <input id="pwd" name="pwd" type="password" class="login_input" value="" />
					<input type="hidden" name="username" id="username"/>
                          	<input id="isPortalisPortal" name="isPortal" type="hidden" value="1" />
          					<input id="data" name="data" type="hidden" value="" />
                  </li>
                  <li>
                    <input name="validateCode" id="validateCode" type="text"   class="check fl"  maxlength="4" value=""/>
                  	<span class="yanzeng fr">
                          	<img src="<%= Common.getInterfaceUrl(request.getServerName(),"SSO","SSO.CODE")%>" onClick="this.src='<%= Common.getInterfaceUrl(request.getServerName(),"SSO","SSO.CODE")%>?r='+new Date().getSeconds()" width="60" height="28" />
                            </span>
                  </li>
                  <li>
                    <input type="submit" class="logBtn" value="确定" onClick="return login_check();" id="loginbuttion" />
                  </li>
                </ul>
                    </form>
                    <!-- 登录结束 -->	
        ...
        <dd class="grayFon youqi">
         ...
         <span class="fr">
		<a href="#" onClick="util.openInFrame('TMS.CENTER');return false;">个人中心</a> / 
		<span id="zxzf" style="display:none" ><a href="#" onClick="util.openInFrame('PORTAL.PAY');return false;"> 支付</a> / </span>
		<a href="http://<%=Common.getSysUrl(request.getServerName(),"CMS") %>/A01/A01050/A01050014/list_1.html" target="_blank" >帮助</a> / 
  ...
  <!--end w1200-->
  <div class="f-nav<%= ("0".equals(usertype)?" parent":"") %><%= ("4".equals(usertype)?" green":"") %>">
    <div class="<%=screenWidthId%>">
      <ul id="menu" >
        	<!-- 一级栏目开始 -->
<%
			...
			if(st==0){
				...
				if(tf.getHref()!=null || tf.getHref().trim().length()==0){
					if("4".equals(tf.getOpentype())){
						//新开页打开moduleurl配置连接
						out.println("<li lmid='"+tf.getId()+"' src='"+tf.getHref()+"' opentype='"+tf.getOpentype()+"' bg='"+tf.getIcon()+"' ><span>/</span><a href='script:void(0);' target='_blank'>"+tf.getText()+"</a></li>");
						...
						out.println("<li lmid='"+tf.getId()+"' src='"+tf.getHref()+"&channelid="+thischannelid+"&cookieid="+RCode.getRCode()+"&regFlg="+regFlg+"' opentype='"+tf.getOpentype()+"' bg='"+tf.getIcon()+"' ><span>/</span><a href='javascript:void(0)' target='_blank' >"+tf.getText()+"</a></li>");
					}else if("3".equals(tf.getOpentype())){
						//在框架内打开moduleurl配置连接
						out.println("<li lmid='"+tf.getId()+"' src='"+tf.getHref()+"' opentype='"+tf.getOpentype()+"' bg='"+tf.getIcon()+"' ><span>/</span><a href='"+tf.getHref()+"' target='iframe01' >"+tf.getText()+"</a></li>");
					}else if("5".equals(tf.getOpentype())){
						//以js方式调用moduleurl配置js方法,js执行src值
						out.println("<li lmid='"+tf.getId()+"' src='"+tf.getHref()+"' opentype='"+tf.getOpentype()+"' bg='"+tf.getIcon()+"' ><span>/</span><a href='javascript:void(0)' >"+tf.getText()+"</a></li>");
					}else{
						//1在框架内打开c3系统moduleurl配置相对地址连接加入channelid,regflag,cookieid参数
						String thischannelid=tf.getId().replaceAll("_",".");
						out.println("<li lmid='"+tf.getId()+"' src='"+tf.getHref()+"&channelid="+thischannelid+"&cookieid="+RCode.getRCode()+"&regFlg="+regFlg+"' opentype='"+tf.getOpentype()+"' bg='"+tf.getIcon()+"' ><span>/</span><a href='javascript:void(0)' target='iframe01' >"+tf.getText()+"</a></li>");
					}
				}
			}
		}
...
<%
	for(int st=0;st<menauthority.size();st++){
		...
		%>
    <div class="s-nav-wrap<%= ("0".equals(usertype)?" parent":"") %><%= ("4".equals(usertype)?" stu":"") %>" id="<%= tf1.getId() %>" style="display:none">
