...
if(requestAlertCode!=null && "true".equals(requestAlertCode)){
	noalert = "true";
}
...
<link href="<%=path %>/space/module/hhhts/css/hs.css?r=<%=dayStamp %>" rel="stylesheet" type="text/css" />
...
$(function(){
	...
	$("#vcode").click(function(){this.src=protocol+sysconfig.SSO+interface_config["SSO.CODE"]+'?r='+new Date().getSeconds();});
	window.setTimeout('$("#vcode").click()',200);
	
	<% if(tuser.getHeadPhoto()!=null && tuser.getHeadPhoto().trim().length()>0){ %>
		$("#userphoto").attr("src",protocol+sysconfig.VFS+interface_config["VFS.PHOTO"]+"<%= tuser.getHeadPhoto()%>?r=<%=Math.round(Math.random()*10000) %>");
	<% } %>
	...
	try{
		if(schoolName.length>7){
			$("#showSchool").html("<a target='_blank' href='"+protocol+sysconfig.SPACE+"/zone/?stype=1&sid="+schoolId+"' title='"+schoolName+"' ><i class='fa fa-home' style='font-size: 16px;'></i> "+schoolName.substring(0,6)+".."+"</a>");
		}else{
			$("#showSchool").html("<a target='_blank' href='"+protocol+sysconfig.SPACE+"/zone/?stype=1&sid="+schoolId+"' title='"+schoolName+"' ><i class='fa fa-home' style='font-size: 16px;'></i> "+schoolName+"</a>");
		}
	}catch(e){}
...
<div class="head_wrap">
  <div class="<%=screenWidthId%>">
    <div class="yj_logo<%= ("4".equals(usertype)?" stu":"") %><%= ("0".equals(usertype)?" parent":"") %>"><img id="indexlogo" src="space/images/logo/logo.png" /> <span><%=spaceName %></span></div>
    <div class="ssou_wrap" >
      <div class="ssou" >
        <div class="ssou_k">
    ...
    <div class="userInfo fr" >
      <dl>
        <dt><!-- 头像及用户中心链接 -->
			<a onClick="util.openIcode('ILEARN.TRENDS');" >
			<img id="userphoto" src="<%=path%>/space/images/default1.png"  />
            </a>
		</dt>
        <dd class="posRe">
		<a class="tit1" onClick="util.openIcode('ILEARN.TRENDS');" title="<%=truename %>" ><%=showname %></a><%= ("4".equals(usertype)?"同学":"") %><%= (("2".equals(usertype)||"3".equals(usertype))?"老师":"") %>
		&nbsp;&nbsp;&nbsp;
		...
		<span id="zxzf" style="display:none" ><a href="#" onClick="window.open(protocol+sysconfig['PORTAL']+interface_config['PORTAL.PAY']);return false;"> 支付</a> / </span>
			...
			if(st==0){
				...
				if(tf.getHref()!=null || tf.getHref().trim().length()==0){
					if("4".equals(tf.getOpentype())){
						...
						String url=tf.getHref();
						if(tf.getHref().indexOf("?")>0){
							url=url+"&channelid="+thischannelid+"&cookieid="+RCode.getRCode()+"&regFlg="+regFlg;
						}else{
							url=url+"?channelid="+thischannelid+"&cookieid="+RCode.getRCode()+"&regFlg="+regFlg;
						}	
						out.println("<li lmid='"+tf.getId()+"' src='"+url+"' opentype='"+tf.getOpentype()+"' bg='"+tf.getIcon()+"' ><span>/</span><a href='javascript:void(0)' >"+tf.getText()+"</a></li>");
						...
						out.println("<li lmid='"+tf.getId()+"' src='"+url+"' opentype='"+tf.getOpentype()+"' bg='"+tf.getIcon()+"' ><span>/</span><a href='javascript:void(0)' target='iframe01' >"+tf.getText()+"</a></li>");
					}
				}
			}
		}
	    	...
	    	for(TreeForm sttree:(List<TreeForm>)tf1.getChildren()){
         	if("2".equals(usertype) || "3".equals(usertype)  || sttree.getC2()==null || sttree.getC2().trim().length()==0 || sttree.getC2().indexOf(eduAndGrade)>-1 ){
				...
				if("4".equals(sttree.getOpentype())){
					...
					out.println("<li lmid='"+sttree.getId()+"' src='"+url+"' opentype='"+sttree.getOpentype()+"' bg='"+sttree.getIcon()+"' >"+splitStr+"<a href='javascript:void(0)' >"+sttree.getText()+"</a></li>");
