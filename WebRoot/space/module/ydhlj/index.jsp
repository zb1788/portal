...
<link href="<%=path %>/space/module/ydhlj/css/global.css?r=<%=dayStamp %>" rel="stylesheet" type="text/css" />
<link href="<%=path %>/space/module/ydhlj/css/top_foot.css?r=<%=dayStamp %>" rel="stylesheet" type="text/css" />
...
var indexlogo =  (portal_config.indexlogo!=""&&portal_config.indexlogo!=undefined)?portal_config.indexlogo:"ydhlj_logo.png";
...
<div class="head_wrap">
  <div class="<%=screenWidthId%>">
    <div class="yj_logo<%= ("4".equals(usertype)?" stu":"") %><%= ("0".equals(usertype)?" parent":"") %>"><img id="indexlogo" src="space/images/logo/ydhlj_logo.png" /> <span><%=spaceName %></span></div>
    <div class="ssou_wrap" >
      <div class="ssou" >
        <div class="ssou_k">
    ...
    <!-- 搜索end-->
    <div class="clHeaderUserBox">
    <p><a href="#" onClick="util.openInFrame('TMS.CENTER');return false;" ><% if(tuser.getHeadPhoto()!=null && tuser.getHeadPhoto().trim().length()>0){ %>
			...
			<% } %></a>
        <a class="clHeaderUserName" href="#" onClick="util.openInFrame('TMS.CENTER');return false;" title="<%=truename %>" ><%=showname %></a><%= ("4".equals(usertype)?"同学":"") %><%= (("2".equals(usertype)||"3".equals(usertype))?"老师":"") %>
        <!-- a class="clHeaderDownloadLink" href="#">下载</a  -->
        <a class="clHeaderLogOut" href="/ssoLoginFail.jsp">退出</a>
    </p>
</div>
...
<%
	...
	for(int st=0;st<menauthority.size();st++){
		...
		if("22".equals(tf.getParentid())){
			if(st==0){
				out.println("<li lmid='"+tf.getId()+"' src='#' ><a href='javascript:void(0)' class='sel' >"+tf.getText()+"<i class=\"clHeaderNavActiveIco\"></i></a></li>");
