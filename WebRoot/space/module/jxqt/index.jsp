...
<%
//替代固定w1200 include file="../../screen_width.jsp"
String screenWidthId = "w1200";
//同时写入cookie强制1200样式
Interface.addCookie("screen_width_id","w1200",response);
%>
<%
...
if(truename.length()>3){
	showname=truename.substring(0,3);
}
...
<link href="<%=path %>/space/module/jxqt/css/global.css?r=<%=dayStamp %>" rel="stylesheet" type="text/css" />
<link href="<%=path %>/space/module/jxqt/css/top_foot.css?r=<%=dayStamp %>" rel="stylesheet" type="text/css" />
<link href="<%=path %>/space/module/jxqt/css/room.css?r=<%=dayStamp %>" rel="stylesheet" type="text/css" />
<link href="<%=path %>/space/module/jxqt/css/font_quan.css?r=<%=dayStamp %>" rel="stylesheet" type="text/css" />
<link href="<%=path %>/space/module/jxqt/css/lrtk.css?r=<%=dayStamp %>" rel="stylesheet" type="text/css" />
...
<style type="text/css">
.f-nav .new_menu{width:680px;height:45px;padding-left:13px;overflow:hidden;}
.f-nav .new_menu li a{padding-left:2px;padding-right:3px;margin-left:3px;margin-right:3px;}
.posRe .link1{width:70px;height:40px;overflow:hidden;float:left}
.posRe .link{width:40px;height:40px;overflow:hidden;float:left}
</style>
...
$(function(){
	...
	if("4"==usertype){
		...
		mainpage="http://"+sysconfig.ILEARN+"/p_home_view.action?r=<%=new Date().getDate()%>";
	}
	//新版需要改logo更换逻辑
...
<div class="head_wrap">
  ...
  <div class="f-nav<%= ("0".equals(usertype)?" parent":"") %><%= ("4".equals(usertype)?" green":"") %>">
    <div class="<%=screenWidthId%>  overflow_h"><img id="indexlogo" class="new_logo" src="space/images/logo.png" />
      <ul class="new_menu" id="menu" >
        	<!-- 一级栏目开始 -->
<%
			...
			if(st==0){
				...
				if(tf.getHref()!=null || tf.getHref().trim().length()==0){
					if("4".equals(tf.getOpentype())){
						//新开页打开moduleurl配置连接
						out.println("<li lmid='"+tf.getId()+"' src='"+tf.getHref()+"' opentype='"+tf.getOpentype()+"' bg='"+tf.getIcon()+"' ><a href='script:void(0);' target='_blank'>"+tf.getText()+"</a></li>");
						...
						out.println("<li lmid='"+tf.getId()+"' src='"+tf.getHref()+"&channelid="+thischannelid+"&cookieid="+RCode.getRCode()+"&regFlg="+regFlg+"' opentype='"+tf.getOpentype()+"' bg='"+tf.getIcon()+"' ><a href='javascript:void(0)' target='_blank' >"+tf.getText()+"</a></li>");
					}else if("3".equals(tf.getOpentype())){
						//在框架内打开moduleurl配置连接
						out.println("<li lmid='"+tf.getId()+"' src='"+tf.getHref()+"' opentype='"+tf.getOpentype()+"' bg='"+tf.getIcon()+"' ><a href='"+tf.getHref()+"' target='iframe01' >"+tf.getText()+"</a></li>");
					}else if("5".equals(tf.getOpentype())){
						//以js方式调用moduleurl配置js方法,js执行src值
						out.println("<li lmid='"+tf.getId()+"' src='"+tf.getHref()+"' opentype='"+tf.getOpentype()+"' bg='"+tf.getIcon()+"' ><a href='javascript:void(0)' >"+tf.getText()+"</a></li>");
					}else{
						//1在框架内打开c3系统moduleurl配置相对地址连接加入channelid,regflag,cookieid参数
						String thischannelid=tf.getId().replaceAll("_",".");
						out.println("<li lmid='"+tf.getId()+"' src='"+tf.getHref()+"&channelid="+thischannelid+"&cookieid="+RCode.getRCode()+"&regFlg="+regFlg+"' opentype='"+tf.getOpentype()+"' bg='"+tf.getIcon()+"' ><a href='javascript:void(0)' target='iframe01' >"+tf.getText()+"</a></li>");
					}
				}
			}
		}
...
%>
    ...
    <div class="userInfo fr" >
      <dl>
        ...
        <dd class="posRe">
		<div class="link1" ><a class="tit1" onClick="util.openInFrame('TMS.CENTER');" title="<%=truename %>" ><%=showname %></a><%= ("4".equals(usertype)?"同学":"") %><%= (("2".equals(usertype)||"3".equals(usertype))?"老师":"") %></div>
		<div class="link" ><a class="tit1" href="http://cmshdkt.jxrrt.cn/mobile/hdkt/index.html">下载</a></div>
		<div class="link" ><a class="tit1" href="/ssoLoginFail.jsp">退出</a></div>
		</dd>
      </dl>
    </div>
    <!--end myInfo-->
