(function($) {
    ...
    $.fn.prettyPhoto = function(pp_settings) {
        ...
        $.prettyPhoto.initialize = function() {
            ...
            pp_images = (isSet) ? jQuery.map($(this).find('sp'),
            function(n, i) {
                if ($(n).attr('data-gal').indexOf(theRel) != -1) return $(n).attr('href')
            }) : $.makeArray($(this).find('img').attr('href'));
            pp_titles = (isSet) ? jQuery.map($(this).find('sp'),
            function(n, i) {
                if ($(n).attr('data-gal').indexOf(theRel) != -1) return $(n).attr('title')
            }) : $.makeArray($(this).find('img').attr('alt'));
            pp_descriptions = (isSet) ? jQuery.map($(this).find('sp'),
            function(n, i) {
                if ($(n).attr('data-gal').indexOf(theRel) != -1) return $(n).attr('title')
            }) : $.makeArray($(this).attr('title'));
            if (pp_images.length > 30) settings.overlay_gallery = false;
            set_position = jQuery.inArray($(this).find('sp').attr('href'), pp_images);
        ...
        $.prettyPhoto.open = function(event) {
            ...
            $pp_pic_holder.fadeIn(function() { (settings.show_title && pp_titles[set_position] != "" && typeof pp_titles[set_position] != "undefined") ? $ppt.html(unescape(pp_titles[set_position])) : $ppt.html('&nbsp;');
                ...
                case 'youtube':
                    ...
                    movie = 'http://www.youtube.com/embed/' + movie_id; (getParam('data-gal', pp_images[set_position])) ? movie += "?data-gal=" + getParam('data-gal', pp_images[set_position]) : movie += "?data-gal=1";
