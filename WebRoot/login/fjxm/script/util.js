...
UtilObj.prototype.makeUrl=function(isyscode,icode){
	...
	try{
		iurl=iurl.replaceAll("$username$",username).replaceAll("$usertype$",usertype).replaceAll("$classId$",classid).replaceAll("$studentId$",stid);
		return protocol+sysurl+iurl;
	}catch(e){
		return protocol+sysurl+iurl;
	}
	...
	$.getScript(protocol+sysconfig["CMS"]+interface_config[icode],function(){ 
		...
		if(result!=null && result.infosList){
			...
			$(result.infosList).each(function(num){
				...
				rdata.push({"id":$(this).attr("infoid"),"title":$(this).attr("topic"),"desc":$(this).attr("description"),"url":url,"pic":img,"time":$(this).attr("pubtime"),"tag":$(this).attr("tag"),"type":$(this).attr("contenttype"),"mark":"","top":$(this).attr("iscommend"),"a9":$(this).attr("a9"),"a10":$(this).attr("a10"),"a15":$(this).attr("a15"),"data":$(this)[0]});
				rnum++;
			});
			callbackhander(rdata);
		}else{
...
UtilObj.prototype.getCommonCMS=function (showid,icode,ilength,type,templete,filterCMS,shandel){
