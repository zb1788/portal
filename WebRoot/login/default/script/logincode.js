...
//二维码通过
codeLoginObj.prototype.codeIn=function(codeid){
	//window.location.href = decodeURIComponent(myjson.url);
	//改为自组url
	window.location.href = util.makeUrl("SSO","SSO.CHANGEUSE")+"?key="+codeid+"&gjpt="+$("#gjpt").val();
}
//二维码过期
codeLoginObj.prototype.checkResult=function(myjson){
	if(myjson.flag=="2"){
		//二维码通过
		codelogin.codeIn(codelogin.codeid);
		...
		codelogin.status=false;
		codelogin.codeid=null;
		$("#qrcodeCanvas").html("<br/><br/>二维码过期<br/><a onclick='codelogin.getLoginCode()'>点此刷新二维码</a>");
	}
}
